...
const msgsyncDark: ThemeRegistration = {
  name: 'msgsync-dark',
  type: 'dark',
  colors: {
...
const msgsyncLight: ThemeRegistration = {
  name: 'msgsync-light',
  type: 'light',
  colors: {
    'editor.background': '#00000000',
...
const frameworksList: {
...
}[] = [
  {
    ...
    code: `import { createClient } from '@msgsync/msgsync-js'

const msgsync = createClient(
  process.env.MSGSYNC_URL,
  process.env.MSGSYNC_ANON_KEY
)
...
const msgsync = createClient(
  process.env.NEXT_PUBLIC_MSGSYNC_URL,
  process.env.NEXT_PUBLIC_MSGSYNC_ANON_KEY
)

export default async function Page() {
  const { data: todos } = await msgsync
    .from('todos')
    .select('*')
  ...
  },
  {
    ...
    code: `import 'package:msgsync_flutter/msgsync_flutter.dart';

Future<void> main() async {
  await Msgsync.initialize(
    url: 'YOUR_MSGSYNC_URL',
    anonKey: 'YOUR_MSGSYNC_ANON_KEY',
  );
  runApp(MyApp());
}

final msgsync = Msgsync.instance.client;

final data = await msgsync
    .from('todos')
    .select('*');`,
    lang: 'dart' as const,
  ...
  },
  {
    ...
    code: `<script>
  import { createClient } from '@msgsync/msgsync-js'

  const msgsync = createClient(
    PUBLIC_MSGSYNC_URL,
    PUBLIC_MSGSYNC_ANON_KEY
  )

  let todos = []

  async function loadTodos() {
    const { data } = await msgsync
      .from('todos').select('*')
    todos = data
  }
</script>`,
...
const msgsync = createClient(
  import.meta.env.VITE_MSGSYNC_URL,
  import.meta.env.VITE_MSGSYNC_ANON_KEY
)
...
onMounted(async () => {
  ...
  },
  {
    ...
    code: `<script setup>
const msgsync = useMsgsyncClient()
...
export async function FrameworksSection() {
  const hl = await createHighlighter({
    themes: [msgsyncDark, msgsyncLight],
  ...
  const frameworks = frameworksList.map((fw) => ({
    ...
    icon: fw.icon,
    docsUrl: fw.docsUrl,
    darkHtml: hl.codeToHtml(fw.code, { lang: fw.lang, theme: 'msgsync-dark' }),
    lightHtml: hl.codeToHtml(fw.code, { lang: fw.lang, theme: 'msgsync-light' }),
